...
#        ./crntb-adm.sh uninstall
        echo "Stopping register.7x24hs.com service, begin killing ${PID}"
    ...
    if [ "$PID" != "" ] ; then
        echo "Starting register.7x24hs.com service..."
        nohup python index.py -log_file_prefix=./log/register.log &
        echo $! > pidfile    #record process id to file
        echo 'Startup register.7x24hs.com service success!'
